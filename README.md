...
## v0.9 information-structure affordance map

v0.9 kept both v0.8 three-action Chaos policies fixed and varied only observation reliability and hidden-state persistence over a prospectively frozen 12-cell environment grid. Lower observation reliability strongly expanded branch and genuine three-way opportunity prevalence. At the prespecified low-information/high-volatility anchor (`observation_accuracy=0.45`, `switch_probability=0.30`), branch opportunities reached 57--60% and three-way opportunities 22--26%; both policies also passed the matched local exploitability and value gates. The full v0.9 criterion nevertheless **failed** because higher state-switch probability increased overall branching but did not monotonically increase three-way branching at fixed 55% observation accuracy. The result constrains environmental affordance to a multidimensional interaction rather than a scalar uncertainty quantity. See `validation/V0_9_AFFORDANCE_MAP_FREEZE.md` and `validation/V0_9_AFFORDANCE_MAP_RESULT.md`.
